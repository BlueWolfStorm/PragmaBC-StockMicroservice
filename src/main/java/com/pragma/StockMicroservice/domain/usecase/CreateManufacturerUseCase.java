...
public class CreateManufacturerUseCase implements ICreateManufacturerServicePort {
    ...
    public void createManufacture(Manufacturer manufacturer) {
        if (manufacturer
                .getName()
                .isEmpty()) throw new NoManufacturerNameBlankFieldException();
        else if (manufacturer
                .getDescription()
                .isEmpty()) throw new NoManufacturerDescriptionBlankFieldException();
        else if (manufacturer
                .getName()
                .length() > MAXIMUM_NAME_LIMIT) throw new MaximumLimitExceededException(MAXIMUM_NAME_LIMIT);
        else if (manufacturer
                .getDescription()
                .length() > MAXIMUM_DESCRIPTION_LIMIT)
